...
/* YTL-end */
  ...
  ****************************************************/
  ...
  if (SCF_iter==1){
    Overlap_Cluster_Ss(CntOLP,Cs,MP, &mpi_comm_level1, myworld1, n);
  }

  if (SpinP_switch==1 && numprocs0==1){
    Hamiltonian_Cluster_Hs(nh[0],Hs,MP,0, &mpi_comm_level1, 0, n);
  }
  else{
    for (spin=0; spin<=SpinP_switch; spin++){
      Hamiltonian_Cluster_Hs(nh[spin],Hs,MP,spin, &mpi_comm_level1, myworld1, n);
    } 
  }
  ...
  ****************************************************/
  ...
  if (SpinP_switch==1 && numprocs0==1 && spin==0){
    spin++;
    Hamiltonian_Cluster_Hs(nh[spin],Hs,MP,spin, &mpi_comm_level1, spin, n);
    goto diagonalize; 
  }
  ...
  ***********************************************/
  ...
  if ( strcasecmp(mode,"scf")==0 ){
    ...
    *************************************************/
    ...
    if (myid1==Host_ID){ printf(" Chemical Potential + Shifted Chemical Potential = %8.5lf + %8.5lf = %8.5lf (eV)\n",ChemP_ori*eV2Hartree,CDDF_Shift_ChemP*\
				eV2Hartree,ChemP*eV2Hartree); fflush(stdout); }
    /* YTL-New-end */
  ...
  ****************************************************/
  ...
  if ( strcasecmp(mode,"scf")==0 ){
    ...
    *************************************************/
    ...
    double ChemP_ori=ChemP;
    ChemP=ChemP_ori+CDDF_Shift_ChemP;
    if (myid==Host_ID){ printf(" Chemical Potential + Shifted Chemical Potential = %8.5lf + %8.5lf = %8.5lf (eV)\n",
