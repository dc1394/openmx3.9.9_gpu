...
void Input_std(char *file)
{
  ...
  ****************************************************/
  ...
  if ( strcmp(filepath,"./")==0){
    input_string("System.CurrrentDirectory",filepath,"./");  /* to be consistent with the legacy keyword. */
  }
  ...
  *************************************************************/ 
  ...
  /****************************************************
       The number of electrons for each species
  ****************************************************/
  ...
  if (fp=input_find("<Number.of.Electrons.Species")) {    
    ...
    for (i=0; i<SpeciesNum; i++){
      ...
      fscanf(fp,"%lf", &Number_of_Electrons_Species[j]) ;
    }

    if (! input_last("Number.of.Electrons.Species>") ) {
      /* format error */
      printf("Format error for Number.of.Electrons.Species\n");
      po++;
    }
  ...
  ****************************************************/
  ...
  if      (MD_switch==17){
    MD_switch = 17;
    cellopt_switch = 1; /* OptC1: no constraint for cell vectors */
  }

  else if (MD_switch==18){
    MD_switch = 17;
    cellopt_switch = 2; /* OptC2: angles fixed for cell vectors */
  }

  else if (MD_switch==19){

    MD_switch = 17;
    cellopt_switch = 3; /* OptC3: angles fixed and |a1|=|a2|=|a3| for cell vectors */
    ...
    */
    ...
    MD_switch = 17;
    cellopt_switch = 4; /* OptC4: angles fixed and |a1|=|a2|!=|a3| for cell vectors */
    ...
    */
    ...
    MD_switch = 17;
    cellopt_switch = 5; /* OptC5: no constraint for cell and coordinates */
  }

  else if (MD_switch==33){
    MD_switch = 17;
    cellopt_switch = 6; /* OptC6: cell opt with fixed a3 vector by SD */
  }

  else if (MD_switch==34){
    MD_switch = 17;
    cellopt_switch = 7; /* OptC7: cell opt with fixed a2 and a3 vector by SD */
  }
  ...
  if      (MD_switch==22){
    MD_switch = 18;
    cellopt_switch = 1; /* RFC1: no constraint for cell vectors */
    ...
    MD_switch = 18;
    cellopt_switch = 2; /* RFC2: angles fixed for cell vectors */
    ...
    MD_switch = 18;
    cellopt_switch = 3; /* RFC3: angles fixed and |a1|=|a2|=|a3| for cell vectors */
    ...
    */
    ...
    MD_switch = 18;
    cellopt_switch = 4; /* RFC4: angles fixed and |a1|=|a2|!=|a3| for cell vectors */
    ...
    */
    ...
    MD_switch = 18;
    cellopt_switch = 5; /* RFC5: no constraint for cell and coordinates */
  }

  else if (MD_switch==31){
    MD_switch = 18;
    cellopt_switch = 6; /* RFC6: cell opt with fixed a3 vector by RF */
  }

  else if (MD_switch==32){
    MD_switch = 18;
    cellopt_switch = 7; /* RFC7: cell opt with fixed a2 and a3 vector by RF */
  }
  ...
  *************************************************************************/
  ...
  /* YTL-New-start */
  input_int("CDDF.frequency.grid.total_number",&CDDF_freq_grid_number,10000);
  /* default: 1000 grids */
  /*   100 grids ( Coarse grids in frequency domain, i.e. ( 10.0 - 0.0 )/  100 =   0.1 eV/grid) */
  /*  1000 grids (Regular grids in frequency domain, i.e. ( 10.0 - 0.0 )/ 1000 =  0.01 eV/grid) */
  /* 10000 grids (  Dense grids in frequency domain, i.e. ( 10.0 - 0.0 )/10000 = 0.001 eV/grid) */
  //input_int("CDDF.frequency.grid.total_number",&CDDF_freq_grid_number,10000); /* default: 10000 grids (Dense grids in frequency domain.) */             
  ...
  input_int("CDDF.approach",&CDDF_approach,0);
  /* default: 0. Selecting a method for calculating momentum matrix elements and optical conductivity. */
  /* 0 = Eq. (1)   in Physical Review B 102 (7), 075143 (2020) by Y. T. Lee, C. C. Lee, M. Fukuda, and T. Ozaki. (numerical approach for collinear cases)\
   */
  /* 1 = Eq. (A15) in Physical Review B 98 (11), 115115 (2018) by C. C. Lee, Y. T. Lee, M. Fukuda, and T. Ozaki. (for collinear/non-collinear cases)*/

  input_double("CDDF.shift_ChemP",&CDDF_Shift_ChemP,0.0); /* default shifted chemical potential: 0.0 eV. The unit is eV. */
  CDDF_Shift_ChemP/=eV2Hartree; /* change the shifted ChemP unit from eV to Hartree */
  /* YTL-New-end */
  ...
  /* YTL-New-start */
  input_int("CDDF.printk",&CDDF_printk,0);
  if (CDDF_printk < 0) CDDF_printk = (CDDF_Kspace_grid1*CDDF_Kspace_grid2*CDDF_Kspace_grid3) / 100;
  /* Default: 0. (no printing)                                                                                                                            
     n = 0 : No printing k in optical calculations.                                                                                                       
     n > 0 : Printing k per n step in optical calculations (= k % CDDF_printk).                                                                           
     n < 0 : Printing k at ( total number of k grids % 100 ) for checking progress.                                                                       
  */
  /* YTL-New-end */
  ...
  ****************************************************/
  ...
  if (version_dftD!=2 && version_dftD!=3){
    ...
    if (myid==Host_ID){
      printf("version.dftD should be 2 or 3.\n");
    }
  ...
  ************************************************************/
  ...
  if (MD_switch==18 && cellopt_switch==5){
    for (i=1; i<=3; i++){  
      for (j=1; j<=3; j++){  
	Cell_Fixed_XYZ[i][j] = 0;
      }
  ...
  if (MD_switch==18 && cellopt_switch==6){
  ...
  else if (MD_switch==18 && cellopt_switch==7){
    Cell_Fixed_XYZ[1][1] = 0; Cell_Fixed_XYZ[1][2] = 0; Cell_Fixed_XYZ[1][3] = 0;
  ...
  else if (MD_switch==17 && cellopt_switch==6){
    Cell_Fixed_XYZ[1][1] = 0; Cell_Fixed_XYZ[1][2] = 0; Cell_Fixed_XYZ[1][3] = 0;
    Cell_Fixed_XYZ[2][1] = 0; Cell_Fixed_XYZ[2][2] = 0; Cell_Fixed_XYZ[2][3] = 0;
    Cell_Fixed_XYZ[3][1] = 1; Cell_Fixed_XYZ[3][2] = 1; Cell_Fixed_XYZ[3][3] = 1;
  }
  ...
  else if (MD_switch==17 && cellopt_switch==7){
    Cell_Fixed_XYZ[1][1] = 0; Cell_Fixed_XYZ[1][2] = 0; Cell_Fixed_XYZ[1][3] = 0;
    Cell_Fixed_XYZ[2][1] = 1; Cell_Fixed_XYZ[2][2] = 1; Cell_Fixed_XYZ[2][3] = 1;
    Cell_Fixed_XYZ[3][1] = 1; Cell_Fixed_XYZ[3][2] = 1; Cell_Fixed_XYZ[3][3] = 1;
  }
  ...
  ****************************************************/
  ...
  if (Use_of_Collinear_Restart==1 && Scf_RestartFromFile==1){
	...
	if (myid==Host_ID){
	  printf("scf.restart=c2n is supported only for non-collinear calculations.\n");
	}
	...
	if (myid==Host_ID){
	  printf("Restart files should be obtained from a collinear calculation in case of scf.restart=c2n.\n");
	}
  ...
  ******************************************************/
  ...
  /**************************************************************
                    for XANES calculations
  ***************************************************************/

  input_logical("XANES.gs.fileout",&xanes_gs_fileout,0);
  input_logical("XANES.calc",&xanes_calc,0);
  input_string("XANES.gs.file",xanes_gs_file,"default");
  input_double("XANES.energy.range",&xanes_energy_range,(double)8.0);
  xanes_energy_range = xanes_energy_range/eV2Hartree;
  ...
  if (xanes_calc==1){
    ...
    if (SpinP_switch==0){
      if (myid==Host_ID){
	printf("scf.SpinPolarization should be ON or NC\n");
      }
      MPI_Finalize();
      exit(0);
    }

    if (strcmp("default",xanes_gs_file)!=0){
      ...
      if ((fp1 = fopen(operate,"rb")) != NULL){
        ...
        fread(HOMO_XANES, sizeof(int), 2, fp1);
        fclose(fp1);
      }
      else{
	if (myid==Host_ID){
	...
	exit(0);
      ...
      if (myid==Host_ID){
        printf("Please speficiy a file by xanes.gs.fileout\n");
      }
      MPI_Finalize();
      exit(0);
    }    
...
int Species2int(char Species[YOUSO10])
{
  ...
  if (po==0){
    ...
    printf("in Atoms.SpeciesAndCoordinates, Hubbard.U.values, or Number.of.Electrons.Species\n");
