...
{
    char  name4[YOUSO10];
    char *f1 = name1, *f2 = name2, *f3 = name3, *f4 = name4;
...
void asbessel(int n, double x, double sbe[2])
{
    ...
    case 2:
        ...
        sbe[1] = (9.0 * cos(x)) / x3 - cos(x) / x - (9.0 * sin(x)) / x4 + (4.0 * sin(x)) / x2;
        break;
    ...
    case 3:
        ...
        x4     = x2 * x2;
        x5     = x4 * x;
        sbe[0] = (-15.0 * cos(x)) / x3 + cos(x) / x + (15.0 * sin(x)) / x4 - (6.0 * sin(x)) / x2;
        sbe[1] = (60.0 * cos(x)) / x4 - (7.0 * cos(x)) / x2 - (60.0 * sin(x)) / x5 + (27.0 * sin(x)) / x3 - sin(x) / x;
        break;
    ...
    case 4:
        ...
        x6 = x5 * x;
        sbe[0] =
            (-105.0 * cos(x)) / x4 + (10.0 * cos(x)) / x2 + (105.0 * sin(x)) / x5 - (45.0 * sin(x)) / x3 + sin(x) / x;
        sbe[1] = (525.0 * cos(x)) / x5 - (65.0 * cos(x)) / x3 + cos(x) / x - (525.0 * sin(x)) / x6 +
                 (240.0 * sin(x)) / x4 - (11.0 * sin(x)) / x2;
        break;
    ...
    case 5:
        ...
        x6     = x5 * x;
        x7     = x3 * x4;
        sbe[0] = (-945.0 * cos(x)) / x5 + (105.0 * cos(x)) / x3 - cos(x) / x + (945.0 * sin(x)) / x6 -
                 (420.0 * sin(x)) / x4 + (15.0 * sin(x)) / x2;
        sbe[1] = (5670.0 * cos(x)) / x6 - (735.0 * cos(x)) / x4 + (16.0 * cos(x)) / x2 - (5670.0 * sin(x)) / x7 +
                 (2625.0 * sin(x)) / x5 - (135.0 * sin(x)) / x3 + sin(x) / x;
        break;
    ...
    case 6:
        ...
        x7     = x3 * x4;
        x8     = x4 * x4;
        sbe[0] = (-10395.0 * cos(x)) / x6 + (1260.0 * cos(x)) / x4 - (21.0 * cos(x)) / x2 + (10395.0 * sin(x)) / x7 -
                 (4725.0 * sin(x)) / x5 + (210.0 * sin(x)) / x3 - sin(x) / x;

        sbe[1] = (72765.0 * cos(x)) / x7 - (9765.0 * cos(x)) / x5 + (252.0 * cos(x)) / x3 - cos(x) / x -
                 (72765.0 * sin(x)) / x8 + (34020.0 * sin(x)) / x6 - (1890.0 * sin(x)) / x4 + (22.0 * sin(x)) / x2;
        break;
    }
}
...
void Associated_Legendre(int l, int m, double x, double ALeg[2])
{
  ...
  ******************************************************/
    ...
    if (l == m) {
        ...
        if (cut1 < tmp0)
            dP = ((double)l * x * p0 - (double)(l + m) * p1) / tmp0;
        else
            dP = 0.0;
...
dcomplex Im_pow(int fu, int Ls)
{
    ...
    if (fu != 1 && fu != -1) {
        ...
        if (fu == 1) {
            if (Ls % 2 == 0) {
                if (Ls % 4 == 0)
                    Cres = Complex(1.0, 0.0);
                else
                    Cres = Complex(-1.0, 0.0);
            } else {
                if ((Ls + 1) % 4 == 0)
                    Cres = Complex(0.0, -1.0);
                else
                    Cres = Complex(0.0, 1.0);
            }
        }
...
void Get_Grid_XYZ(int GN, double xyz[4])
{
    ...
    xyz[1] = (double)n1 * gtv[1][1] + (double)n2 * gtv[2][1] + (double)n3 * gtv[3][1] + Grid_Origin[1];
    xyz[2] = (double)n1 * gtv[1][2] + (double)n2 * gtv[2][2] + (double)n3 * gtv[3][2] + Grid_Origin[2];
    xyz[3] = (double)n1 * gtv[1][3] + (double)n2 * gtv[2][3] + (double)n3 * gtv[3][3] + Grid_Origin[3];
}
