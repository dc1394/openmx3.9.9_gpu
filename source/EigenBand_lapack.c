...
void Eigen_HH(dcomplex** ac, double* ko, int n, int EVmax, int ev_flag)
{
    ...
    ****************************************************/
    ...
    if (dste_flag == 0)
        lapack_dstegr2(n, EVmax, D, E, ko, ac);
    else if (dste_flag == 1)
        lapack_dstedc2(n, D, E, ko, ac);
    else if (dste_flag == 2)
...
void Eigen_zheev(dcomplex** A, double* W, int N0)
{
    ...
    (JOBZ, UPLO, &N, A0, &N, W, WORK, &LWORK, RWORK, &INFO);
...
int Eigen_zheevx(dcomplex** A, double* W, int N0, int MaxN, int ev_flag)
{
    ...
    F77_NAME(zheevx, ZHEEVX)
    (JOBZ1, RANGE, UPLO, &N, A0, &LDA, &VL, &VU, &IL, &IU,
