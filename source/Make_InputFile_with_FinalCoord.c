    ...
    if (fp2!=NULL){
      ...
      while (fgets(st,800,fp2)!=NULL){
        ...
        /* MD.Init.Velocity && VerletXYZ (NEV) */
                 ...
                 strncmp(st1,"<md.init.velocity",17)==0
		 && ( MD_switch==1   ||  // NVE
                      MD_switch==2   ||  // NVT_VS 
                      MD_switch==9   ||  // NVT_NH
        ...
        else if (strncmp(st1,"<md.init.velocity.prev",22)==0){
        ...
        /* NH.R, NH.nzeta, and NH.czeta */

        else if (strncmp(st1,"nh.r",4)==0){
          /* nothing is done .*/
	}

        else if (strncmp(st1,"nh.nzeta",8)==0){
          /* nothing is done .*/
	}

        else if (strncmp(st1,"nh.czeta",8)==0){
          /* nothing is done .*/
	}
        ...
        else if (Atoms_UnitVectors_flag==0 && strncmp(st1,"<atoms.unitvectors",18)==0){
...
void Make_InputFile_with_FinalCoord_NEGF(char *file, int MD_iter)
{
  ...
  if (myid==Host_ID){
    ...
    /* add md.init.velocity if it was not found. */
